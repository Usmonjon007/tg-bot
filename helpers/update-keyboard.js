...
const keyboardUpdate = async (channelId, channelMessageId, message) => {
  ...
  if (post.commentsEnable === true) {
    if (post.groupMessageId) {
      votesKeyboardArray.push({
        ...
        url: `https://t.me/c/${channel.groupId.toString().substr(4)}/${
          channel.settings.showStart === "top"
            ? 2
            : post.groupMessageId + 1000000
        }?thread=${post.groupMessageId}`,
      });
    } else {
      if (
        new Date().getTime() >
        new Date(post.createdAt.getTime() + 1000 * 30).getTime()
      ) {
  ...
  if (
    !post.keyboardNextUpdate ||
    new Date().getTime() > post.keyboardNextUpdate.getTime()
  ) {
    return Promise.race([
      telegram
        .callApi(methodUpdate, optsUpdate)
        .then(() => {
        ...
        .catch((error) => {
          if (error.parameters && error.parameters.retry_after) {
            db.Post.findByIdAndUpdate(post, {
              keyboardNextUpdate: new Date(
                new Date().getTime() + 1000 * error.parameters.retry_after
              ),
            }).then(() => {});
          }
          if (error.code === 400) {
...
async function checkPostForUpdate() {
  ...
  for (const post of findPost) {
    promises.push(
      keyboardUpdate(post.channel.channelId, post.channelMessageId)
    );
  }
