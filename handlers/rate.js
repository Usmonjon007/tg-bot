...
const { keyboardUpdate } = require("../helpers");
...
composer.action(
  ...
  async (ctx) => {
    ...
    if (post.rate.votes.length === 2)
      post.rate.score =
        post.rate.votes[0].vote.length - post.rate.votes[1].vote.length;
    ...
    if (updateResult.error && updateResult.error !== "timeout") {
      ...
      ctx.state.answerCbQuery = [
        resultText +
          ctx.i18n.t("rate.vote.rated_limit", {
            rateName,
            reactList: reactListArray.join("\n"),
          }),
        true,
      ];
    } else {
