...
const { keyboardUpdate, findEmojis } = require("../helpers");

const composer = new Composer();

composer.on("channel_post", async (ctx, next) => {
  if (ctx.session.channelInfo.settings.type === "never") return next();
  if (ctx.session.channelInfo.settings.type === "one")
    ctx.session.channelInfo.settings.type = "never";
  ...
  if (!emojis && ctx.session.channelInfo.settings.type === "request")
    return next();
  ...
  if (ctx.session.channelInfo.settings.commentsType === "never")
    post.commentsEnable = false;
  if (ctx.session.channelInfo.settings.commentsType === "one")
    ctx.session.channelInfo.settings.commentsType = "never";

  await post.save();

  const updateResult = await keyboardUpdate(
  ...
  if (
    ...
    !ctx.channelPost.forward_from_message_id
  ) {
    const botMember = await ctx.tg.getChatMember(
      post.channel.channelId,
      ctx.botInfo.id
    );
      ...
      for (const admin of post.channel.administrators) {
        ...
        if (adminUser) {
          ...
          await ctx.tg
            .sendMessage(
              admin.user,
              ctx.i18n.t("error.cant_edited", {
                postLink: `https://t.me/c/${ctx.chat.id.toString().substr(4)}/${
...
composer.on("message", async (ctx, next) => {
  if (
    ...
    ctx.message.forward_from_chat.id === ctx.message.sender_chat.id
  ) {
    const channel = await ctx.db.Channel.findOne({
      channelId: ctx.message.forward_from_chat.id,
    });
    const post = await ctx.db.Post.findOne({
      channel,
      channelMessageId: ctx.message.forward_from_message_id,
    });
